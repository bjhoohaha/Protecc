...
class FireBaseInit:
    ...
    def addPacket(self, source_ip, destination_ip, protocol, length, info, uid) :
        data = {
            "createdAt": '{:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()),
            "updatedAt": '{:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()),
            "sourceIp": source_ip,
        ...
        self.db.child('users/' + uid).push(data, self.getUserIDToken())
...
try:
    ...
    uid = input()
    fb = FireBaseInit(config, "admin@protecc.com", "adminadminadmin")
