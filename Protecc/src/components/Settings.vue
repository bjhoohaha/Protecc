  ...
  <v-container fluid>
    ...
    <v-layout row wrap>
      ...
      <v-flex xs12>
        ...
        <v-switch v-model="infinity"
            :label="`Infinity Mode : ${onOff(infinity)}`"
            @change="changeInfinityMode()"></v-switch>
      </v-flex>
      ...
      <v-flex xs11 class="Patua" id="number">
        ...
        <!-- text input -->
        <v-text-field type="number" solo flat id="text-center"
            v-model:value="input" v-if="!infinity && edit"
            :rules="[value => !!value || 'Required.']"></v-text-field>
      </v-flex>
      <v-flex xs1>
        <!-- edit button -->
        <v-btn fab small depressed @click="edit = true"
            v-if="edit == false && infinity == false">
          <v-icon>edit</v-icon>
        </v-btn>
        <!-- save button -->
        <v-btn fab small depressed @click="submitNumber()"
    ...
    <v-layout row wrap>
      ...
      <v-flex xs12>
        <v-switch v-model="rules" :label="`Rules Mode: ${onOff(rules)}`"
